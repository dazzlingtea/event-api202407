...
public class EventUser {
    ...
    // NOT NULL 을 하지 않는 이유: SNS로그인한 회원, 인증번호만 받고 회원가입 완료하지 않은 사람처리
    @Column(length = 500)
    private String password;

    @Enumerated(EnumType.STRING)
    @Column(nullable = false)
    @Builder.Default // 값을 초기화한 경우 추가하면 좋은 어노테이션
    private Role role = Role.COMMON; // 권한

    private LocalDateTime createAt; // 회원가입 시간

    // 이메일 인증을 완료했는지 여부
    // 엔터티에 boolean타입을 사용하면 실제 DB에는 0, 1로 저장됨에 주의 (String이나 ENUM 해야만  true, false)
    @Setter
    @Column(nullable = false)
